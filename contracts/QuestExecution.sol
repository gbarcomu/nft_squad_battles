...
contract QuestExecution {
    ...
    function executeQuest(bytes4 playerSquad, bytes4 gmSquad) internal returns (bool) {

        uint8[3] memory battles;
        bool[3] memory winners;
        (battles[0], battles[1], battles[2]) = getPseudoRandomNumbers();
        for(uint8 i = 0; i < 3; i++) {
            winners[i] = battles[i] < winningOddsCalculator(playerSquad[i], gmSquad[i]);
        }
        emit BattleWinners(winners[0], winners[1], winners[2]);
        return ((winners[0] ? 1 : 0) + (winners[1] ? 1 : 0) + (winners[2] ? 1 : 0) > 1);
    }

    function getPseudoRandomNumbers() internal returns (uint8, uint8, uint8) {
        uint8[3] memory battles;
        uint8 MODULUS = 100;
        for(uint8 i = 0; i < 3; i++) {
            battles[i] = uint8(uint(keccak256(abi.encodePacked(block.timestamp, blockhash(block.number - i - 1)))) % MODULUS);
        }
        emit PseudoRandomNumbers(battles[0], battles[1], battles[2]);
        return (battles[0], battles[1], battles[2]);
    }

    function winningOddsCalculator(bytes1 playerUnit, bytes1 gmUnit) internal pure returns (uint8) {
